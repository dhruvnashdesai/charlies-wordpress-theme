 ...
 * Cart Page
 *
 * Theme override of WooCommerce's cart/cart.php.
 *
 * Charlie's customization: the coupon field is lifted out of the table's
 * `.actions` row into its own `.cart-coupon` block rendered after the table
 * (still inside `.woocommerce-cart-form`, so it shares the cart nonce and the
 * apply-coupon submit keeps working). The `.actions` row keeps only the
 * "Update cart" button. Everything else is verbatim from core so behaviour and
 * hooks are unchanged. Keep @version in sync with core to avoid the
 * "outdated template" status notice.
 *
...
<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<?php
	// Charlie's: coupon lifted out of the .actions row into its own block under
	// the cart. Still inside .woocommerce-cart-form, so it shares the cart nonce
	// above and the apply-coupon submit works unchanged.
	if ( wc_coupons_enabled() ) {
		?>
		<div class="cart-coupon">
			<label for="coupon_code" class="cart-coupon__label"><?php esc_html_e( 'Have a coupon?', 'woocommerce' ); ?></label>
